...
pred_pose_photo = f"/home/aut/thesis_utils/output_analysis/comparison/{dataset}/{scene}/PhotoSLAM/"
...
pred_pose_orbslam = f"/home/aut/thesis_utils/output_analysis/comparison/{dataset}/{scene}/ORB_SLAM3/"



## PREPARE DATA FOR PLOTTING ##
# GSICP
pred_gsicp = pred_gsicp_txt[:, [3, 7, 11]]  # Extract tx, ty, tz
gt_gsicp = gt_gsicp_txt[:, [3, 7, 11]]  # Extract tx, ty, tz
# Loopy-SLAM
...
#plt.plot(gt_gsicp[:, 0], gt_gsicp[:, 1], 'w--', label="GS-ICP-SLAM GT")
...
len_traj = np.max([len(err_gsicp), len(err_loopy), len(err_rtabmap)])
